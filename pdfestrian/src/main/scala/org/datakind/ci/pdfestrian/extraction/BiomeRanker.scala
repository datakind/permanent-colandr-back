...
/**
  ...
  */
class BiomeRanker(distMap : Map[String,Int], length : Int) {
  ...
  def aidToFeature(aid : AidSeq) : Seq[BiomeLabel] = {
...
object BiomeRanker {
  ...
  def main(args: Array[String]): Unit = {
    val distribution = AidSeq.load(args.head).toArray.filter(_.biome.nonEmpty).flatMap(_.biome.map{ b => BiomeMap(b.biome)}.distinct).groupBy(a => a).map{a => a._1 -> a._2.length}
    val count = AidSeq.load(args.head).toArray.count(_.biome.nonEmpty)
    val extractor = new BiomeRanker(distribution, count)
